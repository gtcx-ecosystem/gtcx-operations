...
> How the GTCX Operations system works.

This folder contains system design, data models, design principles, and architectural decisions.
...
| [System Architecture](./README.md#system-architecture) | Full system overview (see below) |

### Subfolders

| Subfolder | Purpose | Contents |
|-----------|---------|----------|
| `decisions/` | Architecture Decision Records (ADRs) | `.gitkeep` — ADRs created as needed |
| `data-models/` | Database schemas, content models | `.gitkeep` — models documented as needed |
| `deployment/` | Deployment architecture, environments | `.gitkeep` — docs created when infra is provisioned |
| `monitoring/` | Observability, alerting, SLOs | `.gitkeep` — docs created after first production deploy |
| `principles/` | Design principles | [WhatsApp-First Communications](./principles/whatsapp-first.md), [Audience-Segmented Communications](./principles/audience-segmented-communications.md) |
| `global-south/` | Offline-first, low-bandwidth design | `.gitkeep` — constraints documented as needed |
...
|-----|-------|----------|--------|
| **Gmail** | `gmail.send`, `gmail.readonly` | Send investor updates, legal notices | `03-platform/src/utils/gmail-client.ts` |
| **Calendar** | `calendar`, `calendar.events` | Schedule follow-ups, board meetings | `03-platform/src/utils/calendar-client.ts` |
| **Contacts** | `contacts` | Sync CRM to Google Contacts | `03-platform/src/utils/contacts-client.ts` |
| **Drive** | `drive`, `drive.file` | Store contracts, pitch decks | `03-platform/src/utils/drive-client.ts` |
| **Sheets** | `spreadsheets` | Budget dashboards, investor reports | `03-platform/src/utils/sheets-client.ts` |

**Auth:** JWT-based service account (`03-platform/src/utils/google-auth.ts`) with domain-wide delegation for `ops@gtcx.trade`.
