 ...
 * Run: `pnpm ecosystem:self-heal` or `tsx scripts/ecosystem-self-heal.ts`
 */
...
function ensureGitignore(repo: string): string[] {
  const fixes: string[] = [];
...
function ensureAgentInfra(repo: string): string[] {
  const fixes: string[] = [];
...
function ensureRoadmapIndex(repo: string): string[] {
  const fixes: string[] = [];
  ...
  if (!existsSync(readmePath)) {
    ...
    if (existsSync(roadmapDir)) {
      ...
      fixes.push("Created docs/roadmap/README.md index");
    }
  }
  return fixes;
}

function healRepo(repo: string): HealResult {
  const fixes: string[] = [];
