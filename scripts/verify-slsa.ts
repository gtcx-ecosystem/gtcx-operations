...
function checkReleaseWorkflow(repo: string): SlsaCheck {
  ...
  const checks = [];
  if (hasProvenance) checks.push("provenance");
  if (hasSbom) checks.push("SBOM");
  if (hasCosign) checks.push("cosign");
  if (hasAttestation) checks.push("attestation");
...
function checkCiWorkflow(repo: string): SlsaCheck {
...
function checkSbom(repo: string): SlsaCheck {
  const sbomPaths = [
...
function checkProvenance(repo: string): SlsaCheck {
...
function checkCosign(repo: string): SlsaCheck {
...
function checkReproducibleBuild(repo: string): SlsaCheck {
  ...
  return {
    ...
    detail: existsSync(scriptPath) ? "scripts/reproducible-builds.sh found" : "No reproducible build script",
  };
}

function verifyRepo(repo: string): SlsaCheck[] {
...
function generateMarkdown(repo: string, checks: SlsaCheck[]): string {
