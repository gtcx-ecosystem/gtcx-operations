...
function loadHistory(repo: string): BenchmarkResult[] {
...
function benchmarkRepo(repo: string): BenchmarkResult {
  ...
  if (repo === "baseline-os") {
    try {
      const start = Date.now();
      execSync("node packages/baselineos/dist/cli/bin.js status", { cwd: repoPath, stdio: "ignore", timeout: 60000 });
...
function analyzeTrend(current: BenchmarkResult, history: BenchmarkResult[]): Trend[] {
  const previous = history[history.length - 1];
  if (!previous) return [];

  const trends: Trend[] = [];

  const metrics: Array<{ key: keyof BenchmarkResult; label: string; threshold: number }> = [
  ...
  for (const { key, label, threshold } of metrics) {
    ...
    if (deltaPercent > threshold) status = "regressed";
    else if (deltaPercent < -threshold) status = "improved";
...
function generateMarkdown(results: Array<{ repo: string; result: BenchmarkResult; trends: Trend[] }>): string {
  ...
  for (const { repo, result } of results) {
    ...
    md += `| Metric | Value |\n`;
    md += `|--------|-------|\n`;
    if (result.buildTime > 0) md += `| Build | ${result.buildTime}ms |\n`;
    if (result.testTime > 0) md += `| Test | ${result.testTime}ms |\n`;
    if (result.layerSweepTime > 0) md += `| Layer sweep | ${result.layerSweepTime}ms |\n`;
    if (result.bundleSize > 0) md += `| Bundle size | ${(result.bundleSize / 1024 / 1024).toFixed(2)}MB |\n`;
    md += `\n`;
  }
...
function main() {
  ...
  const results: Array<{ repo: string; result: BenchmarkResult; trends: Trend[] }> = [];
