...
function scanRepo(repo: string): RepoSnapshot {
  return {
    name: repo,
...
function generateAlerts(current: RepoSnapshot[]): Alert[] {
  const alerts: Alert[] = [];
...
function generateTrends(current: RepoSnapshot[], previous: RepoSnapshot[] | null): Trend[] {
  if (!previous) return [];
  const trends: Trend[] = [];
...
function generateMarkdown(report: TrendReport): string {
...
function main() {
  ...
  const report: TrendReport = { generatedAt: new Date().toISOString(), repos, alerts, trends };
