...
function scoreFreshness(name: string, version: string): number {
  const latestMajor = LATEST_MAJORS[name];
  if (latestMajor === undefined) return 100;
  const { major } = parseVersion(version);
  return major >= latestMajor ? 100 : Math.max(0, 100 - (latestMajor - major) * 25);
}

function analyzeRepo(repo: string): { node: RepoNode; edges: DependencyEdge[] } {
  ...
  if (!existsSync(pkgPath)) return { node, edges };
...
function generateMarkdown(nodes: RepoNode[], edges: DependencyEdge[]): string {
  ...
  // Version conflicts
  const conflicts: Array<{ package: string; versions: string[]; repos: string[] }> = [];
  const byPackage: Record<string, Array<{ repo: string; version: string }>> = {};
  ...
  for (const n of nodes) {
    ...
    try {
      ...
      for (const [dep, version] of Object.entries({ ...pkg.dependencies, ...pkg.devDependencies })) {
        if (!byPackage[dep]) byPackage[dep] = [];
