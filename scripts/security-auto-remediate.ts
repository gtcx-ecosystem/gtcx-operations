...
function parseAuditReport(): Finding[] {
...
function generateRemediationPlan(findings: Finding[]): RemediationPlan[] {
  const byRepo: Record<string, Finding[]> = {};
...
function generateMarkdown(plans: RemediationPlan[]): string {
...
function executeFixes(plans: RemediationPlan[]): void {
  if (DRY_RUN) {
