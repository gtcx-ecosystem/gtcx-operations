...
const EVIDENCE_MANIFEST = [
    {
        ...
        external_wording: 'Algorithm approval recorded per COORD-ATR-001 agentic attestation artifact',
    ...
    },
    {
        ...
        external_wording: 'Custodian roster recorded per COORD-ATR-001 agentic ceremony model (XR-401-B)',
    ...
    },
    {
        ...
        external_wording: 'Pilot ceremony authorization recorded per COORD-ATR-001 (XR-401-C; agentic path)',
...
function resolveProtocolsRoot() {
    ...
    throw new Error('gtcx-protocols not found. Set PROTOCOLS_ROOT or clone sibling at ../gtcx-protocols');
}
function sha256File(path) {
    return createHash('sha256').update(readFileSync(path)).digest('hex');
}
const protocolsRoot = resolveProtocolsRoot();
const registerPath = join(REPO_ROOT, '01-docs/04-ops/compliance/attestation-register.yaml');
let register;
...
for (const entry of EVIDENCE_MANIFEST) {
    ...
    const payload = JSON.parse(readFileSync(evidencePath, 'utf-8'));
...
//# sourceMappingURL=sync-agentic-attestation.js.map