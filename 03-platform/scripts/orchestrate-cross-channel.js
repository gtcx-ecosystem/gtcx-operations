...
function getPrimaryChannel(contactType) {
    switch (contactType) {
...
function getFollowUpSequence(contactType) {
    switch (contactType) {
        case 'investor':
            return [
                { day: 3, channel: 'email' },
...
const rules = [
    {
        ...
        condition: (thread) => {
            ...
            const alreadyScheduled = thread.follow_ups.some((f) => f.type === 'whatsapp' && f.triggered_by === lastOutbound.id && f.status === 'pending');
...
//# sourceMappingURL=orchestrate-cross-channel.js.map