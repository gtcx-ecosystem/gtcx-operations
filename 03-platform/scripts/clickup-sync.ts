...
async function createClickUpTask(listId: string, task: ClickUpTask): Promise<{ id: string; url: string } | null> {
  if (!API_TOKEN) {
...
${thread.messages.slice(-3).map((m) => 
  `- ${new Date(m.timestamp).toLocaleDateString()} ${m.channel} ${m.direction}: ${m.subject || m.body.substring(0, 60)}`
).join('\n')}
...
*Synced from gtcx-operations threads registry*
    ...
    if (listId) {
      ...
      if (result) {
        ...
        (followUp as any).metadata = { ...(followUp as any).metadata, clickup_task_id: result.id };
