...
import type { EmailLog, EmailTemplate, EmailProviderConfig } from '../03-platform/src/schemas/email.js';
...
function parseArgs(): SendArgs {
  ...
  for (let i = 2; i < process.argv.length; i++) {
    ...
    if (arg.startsWith('--label=')) args.labels!.push(arg.split('=')[1]);
...
if (args.dryRun) {
  ...
  try {
    if (config.provider === 'gmail') {
      // Use Gmail API
      const gmailConfig: GoogleAuthConfig = {
