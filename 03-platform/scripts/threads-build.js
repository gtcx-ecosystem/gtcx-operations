...
function getOrCreateThread(contactId) {
    ...
    if (contact?.type === 'producer')
        purpose = 'general'; // Producer engagement
...
for (const thread of registry.threads.sort((a, b) => new Date(b.last_activity).getTime() - new Date(a.last_activity).getTime())) {
    const statusEmoji = thread.status === 'active' ? '🟢' : thread.status === 'stale' ? '🟡' : thread.status === 'blocked' ? '🔴' : '⚪';
    md += `## ${statusEmoji} ${thread.contact_name}`;
    if (thread.company_name)
        md += ` (${thread.company_name})`;
    md += '\n\n';
    ...
    if (thread.contact_email)
        md += `- **Email:** ${thread.contact_email}\n`;
    if (thread.contact_whatsapp)
        md += `- **WhatsApp:** ${thread.contact_whatsapp}\n`;
    md += '\n';
...
//# sourceMappingURL=threads-build.js.map