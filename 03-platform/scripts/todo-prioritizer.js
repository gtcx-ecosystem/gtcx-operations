...
function scanRepo(repo) {
    ...
    const items = [];
    try {
        const output = execSync(`git grep -n -E "(TODO|FIXME|HACK|XXX):" -- '*.ts' '*.tsx' '*.js' '*.jsx' '*.mjs' '*.py' '*.rs' '*.go' '*.md' || true`, { cwd: repoPath, encoding: "utf-8", maxBuffer: 10 * 1024 * 1024 });
        for (const line of output.split("\n").filter(Boolean)) {
            ...
            let daysOld = 0;
            try {
                const blame = execSync(`git blame -L ${lineNum},${lineNum} --porcelain -- "${file}"`, { cwd: repoPath, encoding: "utf-8" });
...
main();
//# sourceMappingURL=todo-prioritizer.js.map