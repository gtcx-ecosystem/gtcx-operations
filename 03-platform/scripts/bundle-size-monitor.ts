...
function loadHistory(repo: string): BundleRecord[] {
...
function recordResult(result: BundleRecord): void {
  ensureHistoryDir();
  const path = join(HISTORY_DIR, `${result.repo}.jsonl`);
  writeFileSync(path, JSON.stringify(result) + "\n", { flag: "a" });
}

function measureBundle(repo: string): BundleRecord {
...
function generateMarkdown(records: BundleRecord[]): string {
