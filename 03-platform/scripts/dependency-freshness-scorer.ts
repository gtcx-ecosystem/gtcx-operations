...
function getRepoDeps(repo: string): Record<string, DepInfo> {
...
function analyze(): { repos: RepoScore[]; globalDeps: Record<string, DepInfo[]>; duplicates: string[] } {
  const allDeps: Record<string, DepInfo[]> = {};
...
function generateMarkdown(repos: RepoScore[], duplicates: string[]): string {
