...
function loadEcosystemGraph(): DependencyEdge[] {
  ...
  try {
    ...
    if (edgesMatch) {
      ...
      for (const m of edgeMatches) {
        edges.push({ from: m[1], to: m[2], type: m[3] as DependencyEdge["type"] });
      }
    }
...
function inferEdgesFromPackageJson(): DependencyEdge[] {
  const edges: DependencyEdge[] = [];
  const repos = [
...
function analyzeImpact(changedRepo: string, changedPackage?: string): ImpactReport {
