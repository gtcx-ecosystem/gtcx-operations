...
function generateMarkdown(files, broken) {
    ...
    for (const f of files) {
        if (!reposWithDocs[f.repo])
            reposWithDocs[f.repo] = { count: 0, stale: 0, todos: 0 };
        reposWithDocs[f.repo].count++;
        if (f.daysStale > STALE_DAYS)
            reposWithDocs[f.repo].stale++;
...
main();
//# sourceMappingURL=documentation-freshness-tracker.js.map