...
function parseArgs(): ContractArgs {
  const args: Partial<ContractArgs> = {};
  for (let i = 2; i < process.argv.length; i++) {
    const arg = process.argv[i];
    if (arg.startsWith('--template=')) args.template = arg.split('=')[1];
    if (arg.startsWith('--party=')) args.party = arg.split('=')[1];
    if (arg.startsWith('--output=')) args.output = arg.split('=')[1];
    if (arg.startsWith('--effective-date=')) args.effective_date = arg.split('=')[1];
    if (arg.startsWith('--jurisdiction=')) args.jurisdiction = arg.split('=')[1];
    if (arg.startsWith('--term-months=')) args.term_months = arg.split('=')[1];
  }
  return args as ContractArgs;
}
