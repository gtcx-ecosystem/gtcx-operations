...
//
// Usage:
//   npx tsx 03-platform/scripts/provider-org-status.ts
//
// Requires:
//   BASELINE_MASTER_KEY or BASELINE_VAULT_KEY (for vault check)
// =============================================================================
import { GTCX_PROVIDERS, resolveProviderAvailability, } from '@baselineos/vault';
function checkEnvFlag(envVar) {
    return !!(process.env[envVar]?.trim());
}
function getWorkspacesFromEnv(prefix) {
    const workspaces = [];
...
function buildStatus() {
    ...
    return GTCX_PROVIDERS.map((provider) => {
        ...
        let orgCreatedSource = hasKey ? 'vault' : 'unknown';
        let workspaces = [];
...
function printTable(status) {
    ...
    console.log(`${'Provider'.padEnd(maxName)} │ Org? │ Workspaces │ Keys │ Cap? │ Notes`);
    ...
    for (const s of status) {
        ...
        console.log(`${s.name.padEnd(maxName)} │ ${orgIcon}   │ ${String(wsCount).padStart(10)} │ ${String(keyCount).padStart(4)} │ ${capIcon}   │ ${s.notes}`);
    }
...
//# sourceMappingURL=provider-org-status.js.map