...
function parseSince() {
    ...
    if (val.endsWith("d"))
        return `${val.replace("d", "")} days ago`;
    if (val.endsWith("w"))
        return `${val.replace("w", "")} weeks ago`;
    return val;
}
function parseCommit(repo, line) {
...
function getCommits(repo, since) {
    ...
    try {
        ...
        const output = execSync(`git log --since="${since}" --format="${format}"`, { cwd: repoPath, encoding: "utf-8", maxBuffer: 10 * 1024 * 1024 });
        return output.trim().split("\n").filter(Boolean).map(l => parseCommit(repo, l)).filter(Boolean);
    }
    catch {
        return [];
    }
...
function generateMarkdown(commits, since) {
...
main();
//# sourceMappingURL=generate-ecosystem-changelog.js.map