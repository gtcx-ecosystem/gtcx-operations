...
function detectPackageManager(repoPath) {
    ...
    if (existsSync(join(repoPath, "yarn.lock")))
        return "yarn";
    if (existsSync(join(repoPath, "Cargo.lock")))
        return "cargo";
    if (existsSync(join(repoPath, "poetry.lock")))
        return "poetry";
    if (existsSync(join(repoPath, "Pipfile.lock")))
        return "pipenv";
    return null;
}
...
function scanRepo(repo) {
    ...
    const commitsSinceMay1 = countLines(run(repo, "git log --oneline --since='2026-05-01' --format='%h'"));
    ...
    const unpushedCommits = countLines(run(repo, "git log --oneline @{u}..HEAD 2>/dev/null || true"));
    return {
        name: repo,
        tier: "",
        exists,
...
function generateMarkdown(report) {
    ...
    const avgCommits = Math.round(report.reduce((sum, r) => sum + r.commitsSinceMay1, 0) / total);
...
async function main() {
    ...
    for (const { name, tier } of REPOS) {
        ...
        data.tier = tier;
        report.push(data);
        console.log(`${data.commitsSinceMay1} commits, ${data.dirtyFiles} dirty, roadmap=${data.hasRoadmap ? "Y" : "N"}`);
    }
...
//# sourceMappingURL=ecosystem-health.js.map