 ...
 */
export async function authenticate(config: GoogleAuthConfig): Promise<Auth.OAuth2Client> {
  ...
  if (credentials.installed || credentials.web) {
    ...
    throw new Error(
      `OAuth2 token not found. Run the auth flow first:\n` +
 ...
 */
export async function getGmailClient(config: GoogleAuthConfig) {
 ...
 */
export async function getCalendarClient(config: GoogleAuthConfig) {
  const auth = await authenticate(config);
 ...
 */
export async function getPeopleClient(config: GoogleAuthConfig) {
  const auth = await authenticate(config);
 ...
 */
export async function getDriveClient(config: GoogleAuthConfig) {
  const auth = await authenticate(config);
 ...
 */
export async function getSheetsClient(config: GoogleAuthConfig) {
  const auth = await authenticate(config);
